...
export default function TripBudget() {
  ...
  return (
    <AppLayout title="Budget Overview">
      <div className="mb-6">
        <Link to={`/trips/${tripId}`} className="text-slate-600 hover:text-black font-bold flex items-center text-sm transition-colors">
          <ArrowLeft className="w-4 h-4 mr-1.5" /> Back to Itinerary
        </Link>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-slate-100 p-8 rounded-3xl shadow-sm flex flex-col justify-center items-center text-center">
          <div className="flex items-center mb-3">
            <div className="p-3 bg-yellow-100 text-yellow-800 rounded-2xl mr-3 border border-yellow-200">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm h-72 flex justify-center items-center">
          {chartData.length > 0 ? (
            ...
            <div className="text-slate-500 font-semibold">No budget data available</div>
          )}
        </motion.div>
      </div>

      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <h2 className="text-xl font-black tracking-tight text-slate-900 mb-5 flex items-center">
          <TrendingUp className="w-5 h-5 mr-2 text-yellow-600" /> Daily Breakdown
        </h2>
            ...
            <tbody className="divide-y divide-slate-100">
              {budget.byDay?.map(day => (
                <tr key={day.date} className={`${day.overBudget ? 'bg-red-50/30' : ''} hover:bg-yellow-50/40 transition-colors`}>
                  <td className="px-6 py-4 font-bold text-slate-800">{new Date(day.date).toLocaleDateString('en-IN', { weekday: 'short', month: 'short', day: 'numeric' })}</td>
              ...
              {(!budget.byDay || budget.byDay.length === 0) && (
                <tr><td colSpan="3" className="px-6 py-8 text-center text-slate-500 font-semibold">No daily data available.</td></tr>
              )}
            </tbody>
          </table>
        </div>
