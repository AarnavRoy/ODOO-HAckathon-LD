...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#FEFCE8] flex flex-col md:flex-row overflow-hidden font-sans relative">
      {/* Left side: Form */}
      <motion.div 
      ...
      >
        ...
        <div className="w-full max-w-md p-10 bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 p-4 bg-red-50/80 border border-red-100 text-red-600 rounded-2xl text-sm font-semibold flex items-start">
                <div className="bg-red-100 p-1 rounded-full mr-3 shrink-0"><X className="w-4 h-4 text-red-600" /></div>
                {error}
              </motion.div>
            )}
