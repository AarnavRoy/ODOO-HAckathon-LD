...
export default function AccommodationForm({ stop, onUpdate }) {
  ...
  return (
    <div className="mt-2 border border-slate-100 rounded-xl overflow-hidden">
      ...
      {expanded && (
        <div className="px-3 pb-3 pt-2 border-t border-slate-100 space-y-2.5 bg-slate-50/50">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-lg py-1.5 px-2 text-xs font-medium bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="block text-[10px] font-bold text-slate-500 mb-1">Accommodation Cost (₹)</label>
              <input
                type="number"
                min="0"
                value={cost}
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-500 mb-1">Notes / Address</label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              rows={2}
              placeholder="Address, WiFi password, room number..."
              className="w-full border border-slate-200 rounded-lg py-1.5 px-2 text-xs font-medium bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/20 resize-none"
            />
          </div>
