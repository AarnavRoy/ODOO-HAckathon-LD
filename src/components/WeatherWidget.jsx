...
export default function WeatherWidget({ lat, lon, startDate, endDate }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-1.5 text-xs text-slate-400 py-2">
        <span className="w-3 h-3 border border-slate-300 border-t-violet-400 rounded-full animate-spin" />
        Loading weather...
      </div>
    );
  }
  ...
  return (
    <div className="mt-2">
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">🌤️ Weather Forecast</p>
      <div className="flex gap-1 overflow-x-auto pb-1">
        {forecast.map((day) => (
          <div
            key={day.date}
            className="flex-shrink-0 bg-white border border-slate-100 rounded-lg px-2 py-1.5 text-center min-w-[60px]"
          >
